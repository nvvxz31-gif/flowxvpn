...
export default function TabReferral() {
  ...
  return (
    <div className="px-4 pt-16 pb-4">
      ...
      <p className="text-sm mb-4" style={{ color: secondaryText }}>Зарабатывай с FlowX VPN</p>
      ...
      >
        ...
        <p className="text-xs leading-relaxed mb-3" style={{ color: secondaryText }}>
          Вы получаете <span style={{ color: primaryText, fontWeight: 600 }}>50% комиссии</span> со всех пополнений баланса пользователей, которые зарегистрировались по вашей ссылке. Начисление — мгновенно при каждом пополнении.
        </p>
        <div className="grid grid-cols-3 gap-2">
          {[{ label: 'Комиссия', value: '50%' },{ label: 'Выплата', value: 'от ₽500' },{ label: 'Зачисление', value: 'Сразу' }].map(({ label, value }) => (
            <div key={label} className="text-center p-2 rounded-xl" style={{ background: isLight ? 'rgba(0,0,0,0.04)' : 'rgba(255,255,255,0.04)' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 px-3 py-2 rounded-xl text-xs font-mono truncate"
            style={{ background: isLight ? 'rgba(10,132,255,0.08)' : 'rgba(255,255,255,0.05)', color: '#0A84FF', border: '1px solid rgba(10,132,255,0.2)' }}>
            {refLink}
          </div>
